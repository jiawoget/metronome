...
- The v1.1 Repository Formatting Baseline shipped on 2026-08-01; v1.2 Release Assurance & Workflow Closure is the active milestone. The current authorization ends after ordinary Native OpenGSD verification. Product work, dormant-seed activation, a fresh R01, shipping, pull-request creation, final-head review/CI, merge, tagging, archival, and local-main synchronization remain outside scope.
...
- Use native state and roadmap commands for lifecycle mutations, then compare the result with read-only `smart-entry --json`. A genuine unexplained contradiction is a stop condition. During the active v1.2 assurance phase, continue only through Native OpenGSD execution and ordinary verification; do not route to release work from a generic milestone-completion recommendation.

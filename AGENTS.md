...
- The v1.1 Repository Formatting Baseline shipped on 2026-08-01. v1.2 Release Assurance & Workflow Closure remains the current milestone, and its sole Phase 2 passed ordinary Native OpenGSD verification on 2026-08-01. That phase authorization ended at verification; the owner's later 2026-08-01 instruction separately authorizes the bounded release exit through native milestone audit and archival, PR #136 final-head review/CI, merge, and local-main synchronization without another stage-by-stage confirmation. Product work, dormant-seed activation, a fresh R01, and tagging remain outside scope.
...
- Use native state and roadmap commands for lifecycle mutations, then compare the result with read-only `smart-entry --json`. A genuine unexplained contradiction is a stop condition. For the separately authorized v1.2 release exit, capture the verified phase evidence for native milestone audit and archival on the same PR branch, then freeze one final head for CI and read-only review. PR creation alone does not mean the milestone shipped.

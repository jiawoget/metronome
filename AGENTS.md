...
- The v1.1 Repository Formatting Baseline shipped on 2026-08-01. v1.2 Release Assurance & Workflow Closure remains the current milestone, and its sole Phase 2 passed ordinary Native OpenGSD verification on 2026-08-01. That result exhausts the current authorization. Product work, dormant-seed activation, a fresh R01, milestone completion/archival, shipping, pull-request creation, final-head review/CI, merge, tagging, and local-main synchronization remain outside scope.
...
- Use native state and roadmap commands for lifecycle mutations, then compare the result with read-only `smart-entry --json`. A genuine unexplained contradiction is a stop condition. With v1.2 Phase 2 verified, stop at that result; do not route to milestone completion or release work from the generic `milestone-ready` recommendation without fresh owner authorization.

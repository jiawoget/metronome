...
- Every typed GSD dispatch explicitly carries the native-resolved `agent_type`, `model`, `reasoning_effort`, and `fork_turns: "none"` when the active Codex schema supports those fields. Missing exact binding is a fail-closed incompatibility, not permission to inherit parent context or rely on an absent agent-TOML field. Record the declared agent type, model, and effort in observability metadata.
...
- `.planning/config.json` injects `skills/metronome-policy/SKILL.md` only into its explicitly mapped OpenGSD subagents; an `agent_skills` mapping supplies policy context, not dispatch authority, and does not inject the policy into the `$gsd-new-milestone` controller or every lifecycle actor.
- Read-only `gsd-code-reviewer` use remains available.
- Project controllers must not invoke `$gsd-code-review --fix`, `$gsd-audit-fix`, or directly dispatch `gsd-code-fixer`.
- Review findings return through the normal native executor boundary.
- The fixer remains isolated because the installed OpenGSD `gsd-code-fixer` resolves `sandbox_mode = "read-only"` while its prompt requires edits and commits and unconditionally creates a worktree. Do not copy or patch the generated agent. Keep this route prohibited until a later official OpenGSD version supplies `sandbox_mode = "workspace-write"` and respects `workflow.use_worktrees=false`.
- `gsd-debugger` may be dispatched only after the active installed agent is proven to resolve `sandbox_mode = "workspace-write"`; its current working directory, Git top-level, and primary worktree all equal `C:\Users\wsuto\metronome`; and native `agent-skills` resolution includes `skills/metronome-policy`.

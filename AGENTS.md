...
- Before dispatching any executor, run `node scripts/gsd-observability-write.mjs validate-plan --repo . --plan <PLAN.md>`. A nonzero result is `PLAN_BLOCKED`; do not dispatch, auto-retry, or substitute an advisory review.
- Runtime measurement is project-local under ignored `.logs/gsd-observability/`. Controllers own long external waits and invoke the writer before/after named steps; agents never load raw logs into context. Use `node scripts/gsd-observability-summarize.mjs` for compact reporting.

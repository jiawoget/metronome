...
#### 2026-07-22 CAP-03 traceability correction — reviewed product `884805f16d4327e0fa57046f937e38e4f1106540`

**Why the earlier assertion was insufficient:** the CAP-03 table above admitted only the Home/hook/two-component extraction and two tests, and it explicitly excluded domain, service, API, and ID-algorithm changes. The later PLAN and execution receipt record an owner-approved expansion after the changed-file gates rejected UI-owned draft validation/construction and the local ID fallback, but those downstream artifacts cannot amend a capability admission because `RESEARCH.md` is the policy-owned source of Approved Surface. The reviewed product consequently added `parsePracticeGoalDraft` and `createPracticeGoalId` without a matching research-owned admission. [VERIFIED: `skills/metronome-policy/SKILL.md`; `01-01-PLAN.md`; `01-EXECUTION-RECEIPT.md`; `01-VERIFICATION.md`]

**Adjudication:** CAP-03 remains **USE_LOCAL**, amended only to reuse the existing practice-domain validation owner for goal-draft parsing/construction and the existing practice-goal service owner for fail-closed WebCrypto identity creation. This correction supersedes only the contradictory CAP-03 inputs/outputs, dependency decision, Approved Surface, and the original Admission closure that follows; CAP-01, CAP-02, every other CAP-03 constraint, and all historical evidence remain unchanged. No dependency, persistence path, stored-goal schema, compiler target, fallback ID algorithm, generic abstraction, or new owner is admitted. [VERIFIED: owner-approved expansion recorded in `01-01-PLAN.md` and `01-EXECUTION-RECEIPT.md`; reviewed diff]

##### Amended Approved Surface and exact hunk traceability

| Task -> CAP | File / symbol | Permitted hunk class | Evidence and boundary |
|---|---|---|---|
| `01-01-T4` -> `CAP-03 USE_LOCAL` | `src/domain/practice/validation.ts` | Extract the existing inline goal-kind and goal-period Zod enums as `localPracticeGoalKindSchema` and `localPracticeGoalPeriodSchema`, reuse those exact schemas in `localPracticeGoalSchema`, and add only the draft-specific target schemas, `MAX_PRACTICE_GOAL_TARGET`, `PracticeGoalDraftErrorCode`, `PracticeGoalDraftResult`, `PracticeGoalDraftOptions`, private `getPracticeGoalIdentity`, and exported `parsePracticeGoalDraft`. | Required to move the pre-existing Home draft validation/building responsibility into its existing domain owner without duplicating the persisted goal kind/period policy. The parser accepts unknown form values, returns the five recorded stable error categories, preserves `id`/`createdAt` for edits, and obtains new identity/time only through the supplied functions. [VERIFIED: `ef98c287..884805f1` source diff; parser characterization in `tests/unit/practice-goal-repository.test.ts`; finding-free `01-REVIEW.md`] |
| `01-01-T4` -> `CAP-03 USE_LOCAL` | `src/components/home/practice-goal-editor.tsx` | Import and call `parsePracticeGoalDraft`; map its five error codes to the preserved UI messages; pass the existing edit goal or injected ID provider as appropriate. | This is the already-admitted editor component's direct use of the amended domain owner; it replaces the Home-local `validatePracticeGoalDraft` and `buildLocalPracticeGoalFromDraft` bodies rather than creating a parallel path. [VERIFIED: baseline Home source at `ef98c287`; reviewed editor source at `884805f1`; `tests/unit/architecture-boundaries.test.ts`] |
| `01-01-T4` -> `CAP-03 USE_LOCAL` | `src/services/practice-goals/service.ts::createPracticeGoalId` | Add only the exported zero-argument ID provider that calls native `crypto.randomUUID()` and throws when the provider is unavailable. | Required to remove the Home-local `Date.now()`/`Math.random()` fallback while keeping identity creation in the existing goal-service owner. No prefix, alternate entropy source, retry, storage change, or fallback is admitted. [VERIFIED: baseline Home `createDefaultPracticeGoalId`; reviewed service diff; identity characterization in `tests/unit/practice-goal-service.test.ts`; `01-EXECUTION-RECEIPT.md`] |
| `01-01-T4` -> `CAP-03 USE_LOCAL` | `src/hooks/use-practice-session-dashboard.ts` and the already-admitted Goals components | Import the service `createPracticeGoalId`, expose exactly `PracticeSessionDashboardActions.createPracticeGoalId`, return that function from the hook, and pass it through `PracticeGoalsPanel` to `PracticeGoalEditor`. | Required wiring for the single service-owned provider; no hook-local wrapper, fallback, or second identity algorithm is admitted. [VERIFIED: reviewed source/call graph at `884805f1`; `01-01-PLAN.md` key links] |
| `01-01-T4` -> `CAP-03 USE_LOCAL` | `tests/unit/practice-goal-repository.test.ts` | Add only the `parsePracticeGoalDraft` import and characterization cases for valid drafts, the five stable error categories, and edit identity/time preservation. | Admitted test evidence for the amended validation owner; unrelated cleanup in the same test file is not admitted or credited. [VERIFIED: `ef98c287..884805f1` test diff] |
| `01-01-T4` -> `CAP-03 USE_LOCAL` | `tests/unit/practice-goal-service.test.ts` | Add only the `createPracticeGoalId` import, necessary mock cleanup, and the two provider-success/fail-closed cases. | Admitted test evidence for the amended service owner; unrelated syntax/style cleanup in the same test file is not admitted or credited. [VERIFIED: `ef98c287..884805f1` test diff] |

##### Incidental reviewed hunks explicitly excluded

| File / hunk | Disposition | Reason |
|---|---|---|
| `src/domain/practice/validation.ts`: removal of the import-adjacent blank line | **EXCLUDED — no CAP credit** | Formatting-only and unnecessary to relocate goal-draft responsibility. It does not widen the Approved Surface. [VERIFIED: `ef98c287..884805f1` zero-context diff] |
| `src/domain/practice/validation.ts`: removal of `.finite()` from `targetBpmSchema`, both `measureRangeMsSchema` fields, `practiceSessionSchema.durationMs`, `.bpm`, and `.recordingCount`, `sheetRecordingMetadataSchema.durationMs` and `.bpm`, and `localPracticeGoalSchema.target` | **REJECTED — not admitted** | These edits change pre-existing schemas unrelated to goal-draft extraction. The existing record proves no phase-introduced review finding, but it does not prove these removals were required or owner-approved; a clean review is not capability admission. The kind/period schema-reference reuse in the adjacent local-goal hunk is approved separately above. [VERIFIED: `ef98c287..884805f1` zero-context diff; `01-REVIEW.md`; absence from the owner-approved expansion in `01-01-PLAN.md` and `01-EXECUTION-RECEIPT.md`] |
| `src/services/practice-goals/service.ts`: adding `async` to `listPracticeGoals`, `getPracticeGoal`, `savePracticeGoal`, and `deletePracticeGoal` | **REJECTED — not admitted** | The new ID provider does not require changing the four existing CRUD method declarations, and the recorded owner approval names identity creation only. Existing tests/review do not convert unrelated method edits into an approved surface. [VERIFIED: `ef98c287..884805f1` zero-context diff; `01-01-PLAN.md`; `01-EXECUTION-RECEIPT.md`; `01-REVIEW.md`] |

**Reviewed-diff conclusion:** the required domain/service additions now have an exact research-owned `task -> CAP decision -> Approved Surface -> source/test hunk` mapping. The rejected incidental production hunks remain outside CAP-03 and must be absent from any policy-conforming corrected candidate; they receive no slimming credit and cannot be admitted by treating either owner file as a blanket surface. No `NEEDS_SCOPE_DECISION` is required because the existing evidence is sufficient to admit the two required owner responsibilities and to reject the unrelated hunks without widening the capability. [VERIFIED: metronome capability policy; exact reviewed diff and recorded approval]

...
T4 may modify `src/components/home/home-dashboard.tsx`, `src/hooks/use-practice-session-dashboard.ts`, `src/domain/practice/validation.ts`, `src/services/practice-goals/service.ts`, `tests/unit/home-dashboard.test.tsx`, `tests/unit/architecture-boundaries.test.ts`, `tests/unit/practice-goal-repository.test.ts`, and `tests/unit/practice-goal-service.test.ts`, and may add only `src/components/home/practice-goals-panel.tsx` plus `src/components/home/practice-goal-editor.tsx`. It first records a failing architecture-boundary test, then restores behavior while making Home composition-only for Goals. It removes duplicate save/delete callback aliases and the empty-data reference sentinel only where the focused tests prove compatibility. The initial six-file attempt stopped when changed-file debt gates identified UI-owned draft validation/construction and a local ID fallback. The project owner then explicitly approved the narrow expansion to the existing domain validation and goal-service owners. No dependency, persistence path, stored-goal schema change, generic abstraction, or non-WebCrypto ID fallback is admitted.

T5 accepts exactly those ten product/test repair paths plus `xo.config.js` relative to `ef98c28`. The config change only overrides XO 4's ES2024-only `v` preference for TypeScript extensions with the ES2022-compatible `u` requirement; JS/MJS keep the native rule, and no suppression or compiler-target change is admitted. Across `3370d2f..reviewedProductionSha`, the production source inventory is exactly six `M` rows (`format.ts`, `session-comparison.ts`, validation, dashboard hook, goal service, Home) and two `A` rows (the Goals panel/editor). The normalized production total across those eight source paths must be strictly negative; relocation alone does not count. Existing changed files require no Code Health decline/new severe finding, both new files require attributable no-severe results, and Home must be at least `7.0`. T5 is the only permitted evidence retry. Interruption or failure stops without rediscovery, alternate targets, threshold reduction, or automatic replan.

The original 89-line verifier below is historical and must not be rerun unchanged because it assumes four modified-only source rows. T5 instead records a concise combined-range normalized verifier in the run evidence that handles both modified and added files, validates the exact `6M + 2A` source inventory, pins formatter inputs/tool versions, prints per-path and total added/deleted/net counts, and cleans its temporary files. The verifier itself remains runtime evidence under `.logs/`; it is not added to the product repository.
...
- Exact eight-source staged allowlist before changed-file gates and safeguard; exact ten product/test repair paths plus the approved `xo.config.js` correction before the corrected full-hook attempt.

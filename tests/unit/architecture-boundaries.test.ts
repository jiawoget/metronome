...
describe("source architecture boundaries", () => {
  ...
  it("keeps Home as the composition boundary for Practice Goals", () => {
    ...
    const goalsPanelSource = readFileSync(
      "src/components/home/practice-goals-panel.tsx",
      "utf8"
    );
    const goalEditorSource = readFileSync(
    ...
    expect(homeSource).toContain("<PracticeGoalsPanel");
    expect(goalsPanelSource).toContain(
      'from "@/components/home/practice-goal-editor"'
    );
    expect(goalsPanelSource).toContain("<PracticeGoalEditor");
    expect(goalEditorSource).toContain(
      "export function PracticeGoalEditor"
    );

    for (const implementationDetail of [
      "type PracticeGoalDraft",
      "const practiceGoalKinds",
      "const practiceGoalPeriods",
      "const maxPracticeGoalTarget",
      "function PracticeGoalsPanel",
      "function PracticeGoalEditor",
      "function PracticeGoalRow",
      "function createDefaultPracticeGoalDraft",
      "function createDefaultPracticeGoalId",
      "function buildLocalPracticeGoalFromDraft",
      "function validatePracticeGoalDraft",
      "function isPracticeGoalKind",
      "function isPracticeGoalPeriod",
      "function getPracticeGoalLabel",
      "function getGoalEvaluationStatus",
      "function formatPracticeGoalProgress",
      "function getPracticeGoalUnit",
      "function formatPracticeGoalProgressValue",
      "function clampGoalProgressRatio",
      ...
      expect(homeSource).not.toContain(implementationDetail);
    }

    expect(homeSource).not.toContain("data === emptyHomeDashboardData");
    expect(homeSource).not.toContain("onSavePracticeGoal");
    expect(homeSource).not.toContain("onDeletePracticeGoal");
    expect(dashboardHookSource).not.toContain("onSavePracticeGoal");
    expect(dashboardHookSource).not.toContain("onDeletePracticeGoal");
    expect(goalsPanelSource).not.toContain("onSavePracticeGoal");
    expect(goalsPanelSource).not.toContain("onDeletePracticeGoal");
  });

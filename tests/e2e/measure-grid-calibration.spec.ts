...
test("measure grid calibration saves, reloads, isolates sheets, and uses manual offset fallback", async ({
  page
}) => {
  ...
  const firstSheet = await importTestSheet(page, {
    name: "Measure Grid Sheet A"
  });

  await firstSheet.link.click();
  await expect(
    page.getByRole("heading", { name: "Measure Grid Sheet A" })
  ).toBeVisible();
  await expect(
    page.getByTestId("measure-grid-calibration-panel")
  ).toBeVisible();
  await expect(
    page
      .getByTestId("measure-grid-calibration-panel")
      .getByRole("heading", { name: "Measure grid" })
  ).toBeVisible();
  await expect(page.getByTestId("measure-grid-status")).toContainText(
    "Needs calibration"
  );
  await expect(page.getByRole("spinbutton", { name: "Grid BPM" })).toHaveValue(
    "72"
  );
  await expect(page.getByLabel("Grid time signature")).toHaveValue("4/4");
  await expect(
    page.getByRole("spinbutton", { name: "Pickup beats" })
  ).toHaveValue("0");
  await expect(
    page.getByRole("spinbutton", { name: "Measure 1 offset" })
  ).toHaveValue("");
  await expect(
    page.getByRole("button", { name: "Set measure 1 here" })
  ).toBeDisabled();
  await expect(
    page.getByText("No playback timestamp available.")
  ).toBeVisible();
  ...
  await expect(
    page.getByText("Grid BPM must be an integer from 30 to 300.")
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Save grid" })).toBeDisabled();
  await page.getByRole("spinbutton", { name: "Grid BPM" }).fill("84");
  ...
  await expect(
    page.getByText("Pickup beats must be an integer from 0 to 2.")
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Save grid" })).toBeDisabled();
  expect(await readMeasureGrid(page, firstSheet.sheetId)).toMatchObject({
    grid: {
  ...
  const secondSheet = await importTestSheet(page, {
    name: "Measure Grid Sheet B"
  });

  await secondSheet.link.click();
  await expect(
    page.getByRole("heading", { name: "Measure Grid Sheet B" })
  ).toBeVisible();
  await expect(page.getByTestId("measure-grid-status")).toContainText(
    "Needs calibration"
  );

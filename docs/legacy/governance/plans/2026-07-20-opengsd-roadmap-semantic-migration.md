...
> **Status:** Executed historical migration plan, subsequently repaired through whole-branch review. Do not execute these tasks again. Checked steps record completed historical work. The repaired final state uses native dormant seeds as the sole deferred-capability carrier and leaves the primary repository between milestones. A later user decision superseded the pending isolated R-01 pilot after provider verification; the active route is governance merge followed by a real R01 from updated `main`. A late task review after commit `3baa742` invalidated promotion until the future seed-promotion transaction was made atomic; that later contract is recorded below and does not rewrite the checked migration actions.

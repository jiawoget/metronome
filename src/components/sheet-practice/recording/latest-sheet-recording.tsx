...
export function LatestSheetRecording({
  recording
}: {
  ...
  if (!recording) {
    return (
      <div
        data-testid="sheet-latest-recording-empty"
  ...
  return (
    <LatestSheetRecordingLoaded key={recording.id} recording={recording} />
  );
}

function LatestSheetRecordingLoaded({
  recording
}: {
  ...
  return (
    <div
      data-testid="sheet-latest-recording"
    ...
    >
      <div className="min-w-0">
        ...
        <p
          data-testid="sheet-latest-recording-name"
          className="mt-1 truncate text-sm font-semibold"
        >
          {getRecordingDisplayName(recording)}
        </p>
        <p
          data-testid="sheet-latest-recording-duration"
          className="text-xs text-muted-foreground"
        >
          {formatDuration(recording.durationMs)} | {recording.settings.bpm} BPM
